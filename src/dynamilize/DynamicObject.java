...
/**
 * The interfaces that all dynamic objects depend on describe the basic behaviors that dynamic objects have,
 * and the implementation of the interfaces should be generated by the generator.
 * <p>The implementation of this interface should usually not be done externally, but should be generated
 * through {@link DynamicMaker#makeClassInfo(Class, Class[], Class[])}. The implementation classes
 * generated by the generator should satisfy the following behaviors:
 * <ul>
 * <li>Assign a field to save the {@linkplain DataPool data pool} object, which has a private final modifier
 * <li>Assign objects to save fields of the {@linkplain DynamicClass dynamic class}, which have the private final modifier
 * <li>Generate a corresponding constructor for each superclass constructor, and correctly call the
 * corresponding superclass constructor of the superclass. Add two new parameters before the
 * parameters, passing them to the {@linkplain DataPool data pool} and {@linkplain DynamicClass dynamic class}, and assign them to member fields.
 * <li>Implement the various abstract methods of the interface according to the method instructions within this interface
 * </ul>
 * And the constructor of the delegate type should be called from several newInstances in {@link DynamicMaker},
 * otherwise you must provide appropriate constructor parameters for them.
 *
 * @author EBwilson
 */
...
public interface DynamicObject<S> {
	 ...
	 * {@link DataPool#getVariable(String)} method of the data pool and return a value.
	 *
	 ...
	 * <p>The generator implementation should implement this method to call the
	 * {@link DataPool#setVariable(IVariable)} method of the data pool, with its
	 * own parameters passed in separately.
	 *
	...
	/**
	 * Get the value of a member variable of an object. If the variable has not been defined, an exception
	 * will be thrown.
	 *
	 * @param name variable name
	...
	/**
	 * Set a property value for a variable of an object. If the variable cannot be found in the hierarchy, it
	 * will be defined.
	 *
	 * @param name  variable name
	...
	/**
	 * Process the variable with the specified name using the given operator, and set the variable value
	 * based on its calculation result.
	 *
	 * @param name       variable name
	...
	/**
	 * Set the member functions of an object in lambda mode. Function changes to the object in lambda
	 * mode are only effective for this object, and the changes take effect immediately. If the changes
	 * need to be effective for all instances, the dynamic type reference
	 * {@link DynamicClass#visitClass(Class, JavaHandleHelper)}
	 * method should be used to change the behavior of this object.
	 * <p>The generator implementation should implement this method to call the
	 * {@link DataPool#setFunction(String, Function, Class[])}
	 * method of the data pool and pass the parameters in one-to-one correspondence.
	 * <p><strong>Note that for parameters containing generics, regardless of the type parameter, the formal
	 * parameter type is always {@link Object}</strong>
	 *
	...
	/**
	 * Same as {@link DynamicObject#setFunc(String, Function, Class[])}, except that the passed function does not return
	 * a value.
	 */
	...
	/**
	 * Same as {@link DynamicObject#setFunc(String, Function.SuperGetFunction, Class[])}, except that the passed
	...
	/**
	 * Indicate the specified member function of the formal parameter list execution object. If there are
	 * objects derived from types or null values in the parameters, using type to explicitly specify the formal
	 * parameter type can effectively improve execution efficiency.
	 *
	 * @param name Function Name
	 * @param args List of arguments passed to the function
	 * @return function return value
	...
	/**
	 * Directly passing {@link ArgumentList} as a function call for the argument list, unboxing is completed
	 * within the method, making it easier to reference another function in an anonymous function
	 * without unboxing.
	 *
	 * @param name Function Name
