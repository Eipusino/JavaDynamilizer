...
import static dynamilize.classmaker.ClassInfo.BOOLEAN_TYPE;
import static dynamilize.classmaker.ClassInfo.BYTE_TYPE;
import static dynamilize.classmaker.ClassInfo.CHAR_TYPE;
import static dynamilize.classmaker.ClassInfo.INT_TYPE;
import static dynamilize.classmaker.ClassInfo.LONG_TYPE;
import static dynamilize.classmaker.ClassInfo.SHORT_TYPE;
...
public class CodeBlock<R> implements ICodeBlock<R> {
	...
	public final <S> void invoke(ILocal<?> target, IMethod<?, S> method, ILocal<S> returnTo, ILocal<?>... args) {
		codes().add(
				new Invoke<>(target, false, method, returnTo, args)
		);
	}

	public final <S> void invoke(ILocal<?> target, String method, ILocal<S> returnTo, ILocal<?>... args) {
		codes().add(
	...
	public final <S> void invokeStatic(IMethod<?, S> method, ILocal<? super S> returnTo, ILocal<?>... args) {
		codes().add(
				new Invoke<>(null, false, method, returnTo, args)
		);
	}

	public final <S> void invokeStatic(IClass<?> target, String method, ILocal<S> returnTo, ILocal<?>... args) {
		codes().add(
	...
	public final <S> void invokeSuper(ILocal<?> target, IMethod<?, S> method, ILocal<? super S> returnTo, ILocal<?>... args) {
		codes().add(
				new Invoke<>(target, true, method, returnTo, args)
		);
	}

	public final <S> void invokeSuper(ILocal<?> target, String method, ILocal<S> returnTo, ILocal<?>... args) {
		codes().add(
