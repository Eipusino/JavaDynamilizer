...
import dynamilize.classmaker.code.ElementKind;
...
import static dynamilize.classmaker.ClassInfo.DOUBLE_TYPE;
...
public class ASMGenerator extends AbstractClassGenerator implements Opcodes {
	...
	public void visitCodeBlock(ICodeBlock<?> block) {
		...
		if (block.owner().returnType() == ClassInfo.VOID_TYPE) {
			...
			throw new IllegalHandleException("method return a non-null value, but the method is not returning correctly");
	}
		...
		if (value == null) {
			...
			if (componentType.isPrimitive() || componentType == String.class || componentType.isEnum() || componentType.isArray()) {
				...
				if (componentType.isPrimitive()) {
					...
					methodVisitor.visitTypeInsn(Opcodes.ANEWARRAY, ClassInfo.asType(value.getClass()).internalName());
	...
	protected void castAssign(IClass<?> source, IClass<?> target) {
		if (!target.isAssignableFrom(source)) {
			if (!source.isPrimitive() && target.isPrimitive()) {
				...
				methodVisitor.visitTypeInsn(Opcodes.CHECKCAST, target.isArray() ? target.realName() : target.internalName());
		}
	}
