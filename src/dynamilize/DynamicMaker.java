...
import dynamilize.classmaker.code.Label;
...
import static dynamilize.classmaker.ClassInfo.INT_TYPE;
import static dynamilize.classmaker.ClassInfo.OBJECT_TYPE;
import static dynamilize.classmaker.ClassInfo.STRING_TYPE;
import static dynamilize.classmaker.ClassInfo.VOID_TYPE;
...
public abstract class DynamicMaker {
	 ...
	 * @throws IllegalHandleException If the constructor argument cannot match the
	 *                                corresponding constructor, or if there are abstract methods that have not been processed
	 * @see AspectInterface
	 */
	...
	/**
	 * Generate a data pool corresponding to the dynamic type, and the data pool completes the iterative
	 * allocation of data pool information to the superclass based on the dynamic type. At the same time,
	 * generate function/variable entries for all methods and fields generated by the delegate's dynamic
	 * type and place them in the data pool.
	 *
	...
	/**
	 * Obtain the type of the generated dynamic type instance based on the delegated base class and
	 * implemented interface. The type will be generated and put into the pool, and the next retrieval will
	 * directly retrieve the type from the pool.
	 *
	...
	/**
	 * Establish a dynamic class packaging name based on the base class and interface list, with unique (or
	 * sufficiently discrete, avoiding frequent collisions) and invariant packaging names.
	 *
	...
	/**
	 * The method of continuing to delegate the creation of code for the types generated by
	 * dynamic delegation should pass the primary processing up to the superclass.
	 * <p>Similar to the behavior of {@link DynamicMaker#makeClassInfo(Class, Class[], Class[])}, but some of the behavior
	 * needs to be adapted to superclass that already has delegated behavior.
	 */
	@SuppressWarnings({"unchecked"})
	protected <T> ClassInfo<? extends T> makeClassInfoOnDynamic(Class<T> baseClass, Class<?>[] interfaces, Class<?>[] aspects) {
		...
		for (Constructor<?> cstr : baseClass.getDeclaredConstructors()) {
			...
			CodeBlock<Void> code = classInfo.declareConstructor(Modifier.PUBLIC, ParameterInfo.trans(argArr));
		...
		//	}
		{
			...
			IMethod<T, Object> superCaller = (IMethod<T, Object>) classInfo.superClass().getMethod(code.owner().returnType(), code.owner().name(),
					code.owner().parameters().stream().map(ParameterInfo::getType).toArray(IClass[]::new));
	...
	/**
	 * Create a type identifier for dynamic instance types, which should cover all methods in the delegate
	 * target class and implemented interfaces. If a member method of a superclass is not abstract, the
	 * entry to the superclass method should be retained before rewriting the method. The entry to the
	 * superclass method should have a certain identifier for use in generating reference functions for the
	 * base class data pool.
	 * <p>For a given list of base classes and interfaces, the names of the generated dynamic instance base
	 * classes are unique (or sufficiently discrete to make collisions almost impossible).
	...
	protected <T> ClassInfo<? extends T> makeClassInfo(Class<T> baseClass, Class<?>[] interfaces, Class<?>[] aspects) {
		...
		for (Constructor<?> cstr : baseClass.getDeclaredConstructors()) {
			...
			List<ParameterInfo<?>> params = new ArrayList<>(Arrays.asList(ParameterInfo.as(
					0, DynamicClass.class, "$dyc$",
					0, DataPool.class, "$datP$",
					0, DataPool.class, "$basePool$"
			)));
			List<ParameterInfo<?>> superParams = Arrays.asList(ParameterInfo.asParameter(cstr.getParameters()));
			params.addAll(superParams);

			IMethod<?, Void> constructor = classInfo.superClass().getConstructor(superParams.stream().map(ParameterInfo::getType).toArray(IClass[]::new));

			CodeBlock<Void> code = classInfo.declareConstructor(Modifier.PUBLIC, params.toArray(new ParameterInfo[0]));
		...
		//	}
		{
			CodeBlock<Object> code = classInfo.declareMethod(
					...
					"varValueGet",
					OBJECT_TYPE,
					ParameterInfo.trans(STRING_TYPE)
			);
		...
		//	}
		{
			CodeBlock<IVariable> code = classInfo.declareMethod(
					...
					"getVariable",
					VAR_TYPE,
					ParameterInfo.as(0, STRING_TYPE, "name")
			);
		...
		//	}
		{
			CodeBlock<Void> code = classInfo.declareMethod(
					...
					"setVariable",
					VOID_TYPE,
					ParameterInfo.as(0, VAR_TYPE, "var")
			);
			code.assign(code.getThis(), dataPool, stack(DATA_POOL_TYPE));
	...
	private static boolean filterMethod(Method method) {
		// Methods that have already been declared as final will be added to the exclusion list
			...
			if (Modifier.isInterface(superMethod.owner().modifiers())) {
				...
				if (found) {
					c.getMethod(
							...
							superMethod.name(),
							superMethod.parameters().stream().map(ParameterInfo::getType).toArray(IClass[]::new)
					);
				}
			}
	...
	/**
	 * For the mapping process of delegate base classes, it is usually done by directly returning the type
	 * itself. Overwriting of this method is only necessary when private access to the package is required.
	 * <br>Specifically, when delegating private methods of a package, this mapping requires the creation of a
	 * class hierarchy that elevates the private method access modifier to access the protected domains of
	 * each superclass in the base class's package.
	...
	/**
	 * Generate delegation from the base class and implement the given interface list of types. Please refer
	 * to {@link DynamicMaker#makeClassInfo(Class, Class[], Class[])} for the behavior description of the class. The type
	 * description will be generated in this method.
	 * <p>The usual task of this method is to generate the type identifier obtained from makeClassInfo and
	 * load the Java type it represents.
	 *
	 * @param baseClass  Delegate base class
	 * @param interfaces List of implemented interfaces
